...
def _filter_kwargs(allowed: Iterable[str], kwargs: Mapping[str, object]) -> dict[str, object]:
    """Return a new dict containing only keys from `kwargs` that are in `allowed`."""
    return {k: v for k, v in kwargs.items() if k in allowed}


def create_queue(backend: str = "memory", **kwargs: object) -> RequestQueue:
    ...
    if backend_name == "memory":
        ...
        return MemoryPriorityQueue(**filtered)  # type: ignore[arg-type]
    ...
    if backend_name == "redis":
        ...
        filtered = _filter_kwargs(allowed, kwargs)
        return RedisQueue(**filtered)  # type: ignore[arg-type]
